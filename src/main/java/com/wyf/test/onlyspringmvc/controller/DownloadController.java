...
import com.wyf.test.onlyspringmvc.common.util.ZipUtil;
...
public class DownloadController {
	...
	public void downloadAirZip(//
		...
		try {
			// STEP 1：得到要打包的文件
			String[] files = { "/testdownloadfile/测试-文本.txt", "/testdownloadfile/测试-图片.jpg", "/testdownloadfile/测试打包文件夹" };
			List<File> srcPathList = new ArrayList<>();
			for (String file : files) {
				String oriFullFilename = this.getClass().getResource(file).getFile();
