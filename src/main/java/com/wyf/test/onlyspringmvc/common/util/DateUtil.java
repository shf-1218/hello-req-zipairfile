...
public class DateUtil {
	...
	public static final String yrMonDayHrMinSZone_  = 		"yyyy-MM-dd'T'HH:mm:ss.SSS'Z'";
	//public static final String yrMonDayHrMinSZone_  = 		"yyyy-MM-dd'T'HH:mm:ss'Z'";
	...
	public static final String yrMonDayHr_ 			= 		"yyyy-MM-dd HH";
	...
	public static final String yrMonDay3 			= 		"yyyy/MM/dd";
	...
	public static final SimpleDateFormat yrMonDayHrMinSZoneSDF_ 	= 		new SimpleDateFormat(yrMonDayHrMinSZone_);
	...
	public static final SimpleDateFormat yrMonDayHrSDF_ 			= 		new SimpleDateFormat(yrMonDayHr_);
	...
	public static final SimpleDateFormat yrMonDaySDF3 				= 		new SimpleDateFormat(yrMonDay3);
	 ...
	 * @return
	 */
	public static Date parseHourMinute_(String hr_minStr) {
		try {
			synchronized (hrMinSDF_) {
				return hrMinSDF_.parse(hr_minStr);
			}
		} catch (ParseException e) {
			throw new IllegalArgumentException(e);
		}
	 ...
	 * 传入Unix时间戳,转换成日期.
	 * 
	 * 注意Unix时间戳精度到秒,所以传入的unixTime其实是个int,还没到达long,
	 * Unix时间戳和 new java.util.Date().getTime()得到的数字会少三位,即毫秒的三位.
	 * 
	 * @param unixTime
	 * @return
	 * @author Stone
	...
	public static String dateToStrTimeZone(Date date) {
		synchronized (yrMonDayHrMinSZoneSDF_) {
			yrMonDayHrMinSZoneSDF_.setTimeZone(TimeZone.getTimeZone("UTC"));
			return yrMonDayHrMinSZoneSDF_.format(date);
		}
	}
	 ...
	 * @return
	 */
	public static Date fullTimeFromDate(Date date) {
		Calendar cal = Calendar.getInstance();
		cal.setTime(date);
	...
	/**
	 * 把时间的时间部分设置为当天的最大值（不包括毫秒的）
	 * 
	 * @param date
	 * @return
	 */
	public static Date fullTimeFromDateNoMillSec(Date date) {
		Calendar cal = Calendar.getInstance();
		cal.setTime(date);
		cal.set(Calendar.HOUR_OF_DAY, 23);
		cal.set(Calendar.MINUTE, 59);
	...
	/**
	 * 获得年月字符串 格式2017/06/07
	 * 
	 * @param date
	 * @return
	 * @author wby99
	 */
	public static String getSlashYearMonDayStr(Date date) {
		synchronized (yrMonDaySDF3) {
			return yrMonDaySDF3.format(date);
		}
	}
	 ...
	 * @return
	 */
	public static String getHourMinuteStr_(Date date) {
		synchronized (hrMinSDF_) {
			return hrMinSDF_.format(date);
		}
	}
	 ...
	 * <br/>直接加月数不会有什么问题，可以放心使用，如："2017-01-31"加一个月，得到的是"2017-02-28"
	 * @param date
	 * @param amount
	 *            可以传负整数
	 * @return
	 ...
	 * @return 天数(绝对值）
	 */
	public static long getTwoDateIntervalAbs(Date endDate, Date beginDate) {
		return Math.abs(getTwoDateInterval(endDate, beginDate));
	}
	
	/**
	 * 获取endDate和beginDate的相差天数，不取绝对值，endDate若比beginDate小，会取到负数
	 * @param endDate
	 * @param beginDate
	 * @return
	 * @author Stone
	...
	public static List<Date> getRecent7DaysDesc() {
		Date today = getCurDate();
		List<Date> result = new ArrayList<>(7);
		int i = 0;
		while (i < 7) {
			Date d = addDays(today, -i);
			result.add(d);
			i++;
		}
	...
	public static List<Date> getRecent7DaysAsc() {
		List<Date> result = getRecent7DaysDesc();
	...
	/**
	 * 获得传入时间的小时字段，日期时间2015-09-15 01 返回01
	 * 
	 * @param date
	 * @return
	 */
	public static String getHourOfDay(String date) {
		try {
			synchronized (yrMonDayHrSDF_) {
				Date dateHour = yrMonDayHrSDF_.parse(date);
				return hourSDF.format(dateHour);
			}
		} catch (ParseException e) {
			throw new IllegalArgumentException(e);
		}
